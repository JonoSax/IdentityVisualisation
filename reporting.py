...
class Metrics:
    ...
    def __init__(self, plottedIdentities, permissions, uiddf, attribute = None, specificTime = None, sliderRound = None, sliderCluster = None):
    ...
    def getClusterInfo(self, aggDict):
        ...
        dfMod = self.plottedIdentities[self.plottedIdentities["_DateTime"] == self.specificTime].reset_index(drop=True)
    ...
    def getAggregateClusterInfo(self):
        ...
        for clid, dfCluster in self.dfClusters.iterrows():
            ...
            dfAggregate[clid] = self.permissions.loc[[(i, self.specificTime) for i in uids]].sum(0)/len(uids)

        # sort the aggregate plotIDdf by the order of the number of identities which have the permissions 
        # (highest to lowest)
        permissionOrder = self.permissions.loc[(slice(None), self.specificTime), :].sum(0).sort_values(ascending = False)
...
def report_1(plotIDdf, permissions, uiddf, sliderDate, reportName, attribute = None):
    ...
    latestIdentities = dist.identities.loc[(slice(None), dist.specificTime), :].droplevel(1)
    latestPermissions = dist.permissions.loc[(slice(None), dist.specificTime), :].droplevel(1)
    ...
    for attr in sorted(dist.attribute):
        ...
        for ele in sorted(element):
            ...
            if len(ids) > 5:
                ...
                eleSpread = dist.dfDistances[dist.dfDistances.index.isin(ids, level=0) & dist.dfDistances.index.isin([dist.specificTime] , level=1)]
                if len(eleSpread)==0:
                    continue
    ...
    latestPermissions = dist.permissions.loc[(slice(None), dist.specificTime), :].droplevel(1).copy()
    modLatestPermissions = latestPermissions.copy()

    # create the permission data frame from the latest time export
    latestPermissions[[dist.key, "Type"]] = [[m.split("_")[0], "Original"] for m in latestPermissions.index]
    ...
    modLatestPermissions[[dist.key, "Type"]] = [[m.split("_")[0], "Modified"] for m in modLatestPermissions.index]
    ...
    entitleExtract = entitleExtract.merge(dist.identities.loc[(slice(None), dist.specificTime), :].reset_index(), on=dist.key)
    ...
    entitleExtract.loc[entitleExtract["Type"] == "Modified", "_DateTime"] = dist.specificTime + 1      # iterate the latest time to create the "future" permission
...
def report_2(plotIDdf, permissions, uiddf, attribute, sliderRound, sliderCluster, sliderDate, hover_data, reportName):
    ...
    pos = Metrics(plotIDdf, permissions, uiddf, attribute, sliderDate, sliderRound, sliderCluster)
    ...
    addToReport(wsPermAnalysis, ["Element clustering"] + [pos.dfClusters.loc[clID][attribute] for clID in pos.clusterNames], rowNo)
    ...
    for cN, cC in zip(pos.clusterNames, pos.clusterCount):
        dfExclude = dfNet[[c for c in pos.clusterNames if c != cN]].sum(1)/(sum(pos.clusterCount)-cC)
        pos.clusterAgg[cN]
        # diff = pos.clusterAgg[cN] - dfExclude 
        diff = pos.clusterAgg[cN] / dfExclude  # seach for the np.inf to find the unique permissions? 
        # ad = diff.describe()
        # iqr = ad["75%"] - ad["25%"]
        # diffDict[cN] = diff[(diff>iqr*1.5+ad["75%"]) | (diff<ad["25%"]-iqr*1.5)]
        diffDict[cN] = pos.clusterAgg.loc[list(diff[diff == np.inf].index)][cN] # store only the permissions which are unique in each cluster and the % occurence in their cluster
    ...
    addToReport(wsAttrAnalysis, ["Element Clustering"] + [pos.dfClusters.loc[clID][attribute] for clID in pos.clusterNames], rowNo)
